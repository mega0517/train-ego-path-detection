...
**그림 2. 분기부 폐색 검증 예(OSDaR23 함부르크 중앙역, 좌곡선 분기 구간).** 위: 비폐색. 가운데: 폐색 30% — 단일 프레임(빨강)과 기존 RNN(파랑)은 가려진 구간에서 직선으로 외삽되어 정답(노랑 점선)의 좌곡선을 놓치는 반면, 폐색 증강 RNN(초록)은 과거 프레임의 메모리로 곡선 경로를 유지한다(IoU 0.42→0.57). 파랑이 빨강을 정확히 덮는 것은 기존 RNN이 항등임을 보여준다. 아래: 폐색 40% — 회복이 부분적으로만 유지된다.
