...
|---|---|---|---|---|
| brilliant-horse-15RNN (segmentation/resnet18) | 3.89 | 3.89 | 0.9833 | 0.9833 |
| chromatic-laughter-5RNN (regression/efficientnet-b3) | 2.56 | 2.56 | 0.9820 | 0.9814 |

(두 행 모두 커밋된 평가 스크립트의 crop=None 설정에서 결정적으로 재현되는 값이다.)
...
부수적으로, 두 검출 헤드의 시간적 안정성은 서로 비슷한 수준이었다(회귀 2.56 px·0.9820, 세그멘테이션 경계 추출 3.89 px·0.9833). 세그멘테이션이 지터에서 다소 크나 일관성 IoU는 오히려 근소하게 높아, 검출 헤드 선택이 시간적 안정성에 미치는 영향은 결정적이지 않았다.
...
**그림 2. 전두부 폐색 검증 예(OSDaR23 함부르크 중앙역 구내 분기 구간 — 자기 경로가 분기기들을 지나 좌측으로 갈라져 나가는 장면).** 예측 자기 경로를 좌·우 레일 사이의 반투명 영역으로 표시하였다. (a) 비폐색 — 폐색 증강 RNN의 경로(초록)가 정답(노랑 점선)과 일치한다. (b) 폐색 30% — 하단 차폐 구간에서 단일 프레임의 경로(빨강, 기존 RNN도 동일)는 곡선 진로를 놓치고 직선 쪽으로 이탈하는 반면, 폐색 증강 RNN(초록)은 과거 프레임의 메모리로 정답의 좌곡선 경로를 유지한다(IoU 0.39→0.54). 폐색 40%에서는 세 방법 모두 크게 열화된다(표 6).
...
### 4.7 누수 없는 장소 분리 분할에서의 일반화 평가 (OSDaR23)

앞의 프레임별 IoU(표 1)는 RailSem19의 시드 고정 무작위 분할로 산출되는데, 연속 주행 데이터에서는 이 방식이 인접 유사 프레임을 학습·평가에 함께 배치하여 IoU를 낙관적으로 부풀린다. 이를 검토하기 위해 OSDaR23의 라벨 프레임(1,535장)을 **녹화 장소 단위로 분할**하여 같은 장소가 학습·검증·평가에 걸치지 않도록 하고, 세그멘테이션 모델(resnet18)을 재학습하였다. 평가 분할은 짧은 녹화들로 채워 **7개 장소(179 프레임; 본역·수목·공사차량·교량하부·분기·터널진입 등)**를 포함시켰다.

측정 결과, 단일 장소(지하역 210 프레임)로 평가하면 IoU 0.980이 나오나 이는 거의 동일한 프레임의 반복 측정에 가깝고, 위의 **7개 장소 분할에서는 IoU 0.953**으로 이것이 더 정직한 일반화 수치다. 학습에 쓰인 장소에서 재평가하면 0.992로, 분할 절차의 정합성이 확인된다. 체크포인트 선택도 영향이 컸다. 검증 손실이 이른 에폭에서 최저에 도달한 뒤 상승하는 과적합이 관찰되어, 마지막 10% 구간에서만 저장하는 기존 방식은 과적합된 가중치를 저장하였다(IoU 0.933). 전역 검증 최적점을 저장하도록 바꾸면 동일 학습에서 IoU가 0.953으로 회복되었다(동일 조건 재실행 간 편차 0.003 기준, 유의한 차이). 한편 **분기 구간(18_vegetation_switch)에 한정한 IoU는 0.892**로 전체 평균 대비 약 0.06 낮아, 진로가 갈라지는 지점이 여전히 취약함을 정량적으로 뒷받침한다(4.3절).

이 과정에서 자동 라벨의 기하학적 타당성을 점검하는 보조 도구로, 원근 하에서 궤간이 행에 대해 선형이라는 성질을 이용해 교차·궤간 급변 등 비정상 라벨을 표시하는 **궤간 검사**를 도입하였다. 이는 라벨 오류를 사전에 걸러 위와 같은 평가의 신뢰도를 높이는 데 쓰인다. (본 절의 수치는 단일 주행 도메인·소수 장소에 기반하므로, 장소별 성능의 정밀 비교보다는 분할·체크포인트 선택이 보고 수치에 미치는 영향을 보이는 데 목적이 있다.)
