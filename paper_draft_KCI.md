...
**그림 2. 전두부 폐색 검증 예(OSDaR23 뤼벤캄프역 진입 구간).** (a) 비폐색. (b) 폐색 30% — 하단 차폐 구간에서 단일 프레임(빨강)과 기존 RNN(파랑)은 정답(노랑 점선) 안쪽으로 이탈하는 반면, 폐색 증강 RNN(초록)은 과거 프레임의 메모리로 궤도 경계를 유지한다(IoU 0.69→0.77). 파랑이 빨강을 정확히 덮는 것은 기존 RNN이 항등임을 보여준다. (c) 폐색 40% — 세 방법 모두 크게 열화되나 폐색 증강 RNN의 회복이 상대적으로 크다(0.27→0.32).
